seurat_pando_object <- "/storage/singlecell/zz4/fetal_bash/results/NRPC_Pando/NRPC_seurat_object_All_feature_selection_TRUE.rds"
...
df <- read.csv("/storage/singlecell/zz4/fetal_bash/results/multivelo_recover_dynamics_results/NRPC.obs.csv")
rownames(df) <- df$X
...
grn_plot <- grn_plot + ggrepel::geom_label_repel(data = dat[rownames(dat)!=gene,], size = 4,
    ...
    point.padding = 0, label.size = 0, max.overlaps = Inf, segment.color = "grey50",
    min.segment.length = 0, fill = NA)
ggsave(filename = paste(output_dir, label, gene, "_module_metrics.svg", sep = ""), scale = 1, width = 10, height = 10)+ theme(text = element_text(size = 30))   
}
