...
if (class(counts)=="dgCMatrix") {
    ...
    common_cells <- intersect(meta$X,rownames(temp@meta.data))
    temp <- subset(temp,cells = common_cells)
    assign(sam, temp)
...
Multiome_23w4d_FR, Multiome_23w4d_NR,sn_multiome_d59,sn_multiome_d76c,sn_multiome_d76p))
