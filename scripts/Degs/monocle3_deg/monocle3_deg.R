...
input_file <- "/storage/chentemp/zz4/adult_dev_compare/results/merged_rna/merged_rna.rds"
...
meta <- meta[meta$majorclass == cell_type, ]
cells <- meta$X
...
write.table(fit_coefs_region_days_models[, c(-4, -5)], paste(output_dir,
    "fit_coefs_region_days_models.csv", sep = ""), quote = FALSE, sep = ",",
    row.names = FALSE)
write.table(fit_coefs_days_models[, c(-4, -5)], paste(output_dir, "fit_coefs_days_models.csv",
    sep = ""), quote = FALSE, sep = ",", row.names = FALSE)
write.table(fit_coefs_region_models[, c(-4, -5)], paste(output_dir, "fit_coefs_region_models.csv",
    sep = ""), quote = FALSE, sep = ",", row.names = FALSE)

write.table(compare_mod_region, paste(output_dir, "compare_mod_region.csv",
    sep = ""), quote = FALSE, sep = ",", row.names = FALSE)
write.table(compare_mod_days, paste(output_dir, "compare_mod_days.csv",
    sep = ""), quote = FALSE, sep = ",", row.names = FALSE)