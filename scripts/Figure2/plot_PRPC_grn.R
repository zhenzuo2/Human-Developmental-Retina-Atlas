output_dir = "/storage/chentemp/zz4/adult_dev_compare/figures/Figure2/"
label = "PRPC"
...
seurat_object <- readRDS("/storage/chentemp/zz4/adult_dev_compare/results/pando_results/pando_PRPC.rds")
...
p <- p + geom_point(aes(x = UMAP_1, y = UMAP_2, color = TIME,size = Nor_Exp),
    shape = dat$shape) + scale_color_gradient(low = "blue", high = "red") #+scale_size(10)
...
ggsave(filename = paste(output_dir, label, "_grn_plot.tiff", sep = ""),
    plot = p, scale = 1, width = 10, height = 8)
