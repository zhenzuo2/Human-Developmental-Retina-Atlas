...
adata.obsm["Days"] = np.asarray([[x] for x in adata.obs["Days"]])

hs = hotspot.Hotspot(
    adata,
    model="danb",
    layer_key="counts",
    latent_obsm_key="Days",
...
local_correlations.to_csv(
    output_path + "PRPC_hs_local_correlations_Days.csv"
)
module_scores.to_csv(
    output_path + "PRPC_hs_module_scores_Days.csv"
)
joblib.dump(
    hs,
    output_path + "PRPC_hs.create_modules_Days.pkl",
)
adata.write(output_path + "PRPC_hs.adata_Days.h5ad"
)