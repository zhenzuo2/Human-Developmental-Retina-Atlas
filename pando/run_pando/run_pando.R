args <- commandArgs(trailingOnly = TRUE)
# Running Pando for each cell type
library(Signac)
library(Seurat)
library(Pando)
library(BSgenome.Hsapiens.UCSC.hg38)
library(foreach)
library(doParallel)
registerDoParallel(8)
set.seed(0)

cell_type = args[1]
meta_file = args[2]
output_dir = "/storage/singlecell/zz4/fetal_snakemake/results/pando_results/"
input_rna_file = "/storage/singlecell/zz4/fetal_snakemake/results/merged_rna/merged_rna.rds"
input_atac_file = "/storage/singlecell/zz4/fetal_snakemake/results/merged_atac/atac.rds"

# Check if the results already exist or not
dir.create(output_dir, showWarnings = FALSE)

rna <- readRDS(input_rna_file)
meta <- read.csv(meta_file)

cells <- meta$X
rna <- subset(rna, cells = cells)
DefaultAssay(rna) <- "RNA"
rna <- NormalizeData(rna)
rna <- ScaleData(rna)

rna <- FindVariableFeatures(rna, selection.method = "vst", nfeatures = 10000)

seurat_object <- readRDS(input_atac_file)
common_cells <- intersect(colnames(seurat_object), colnames(rna))
seurat_object <- subset(seurat_object, cells = common_cells)
rna <- subset(rna, cells = common_cells)
seurat_object[["RNA"]] <- rna@assays$RNA
seurat_object@meta.data <- cbind(seurat_object@meta.data, rna@meta.data[colnames(seurat_object),
    ])
seurat_object <- FindTopFeatures(seurat_object, assay = "peaks", min.cutoff = "q5")
print(seurat_object)

seurat_object <- initiate_grn(seurat_object)
data(motifs)
seurat_object <- find_motifs(seurat_object, pfm = motifs, genome = BSgenome.Hsapiens.UCSC.hg38)

seurat_object <- infer_grn(seurat_object, peak_to_gene_method = "Signac",
    method = "glm", parallel = TRUE)
saveRDS(seurat_object, paste(output_dir, cell_type, "_seurat_object.rds",
    sep = ""))
seurat_object <- find_modules(seurat_object)
modules <- NetworkModules(seurat_object)
write.csv(modules@meta, paste(output_dir, cell_type, "_modules_meta.csv",
    sep = ""))
