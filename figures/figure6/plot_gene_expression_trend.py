...
for module in [0,1,2,3,4]:
    ...
    for gene in hs.modules[hs.modules==(module+1)].index:
        print(module)
        print(gene)
        ...
        df = adata_result.obs.loc[:,["latent_time","y_hat","latent_time_cut"]].groupby('latent_time_cut').mean()
        y_pred = df.y_hat
        x = df.latent_time
        z_scores = (y_pred-y_pred.mean())/(y_pred.std())
        z_score_global = z_score_global + z_scores
        plt.plot(x, z_scores, color=cols[module],linewidth=0.1)
    z_score_global = z_score_global/len(hs.modules[hs.modules==(module+1)].index)
    plt.plot(x, z_score_global, color='black',linewidth=1)
    plt.xticks([])
    plt.yticks([])
    fig = plt.gcf()
