...
label = "AC"
seurat_pando_object = "/storage/singlecell/zz4/fetal_snakemake/results/pando_results/NRPC_AC_fate_only_seurat_object.rds"
...
muo_data <- get_tf_network(muo_data, tf='FOXN4')
p<- plot_tf_network(
    muo_data, 
    tf = 'FOXN4'
)
ggsave(filename = paste(output_dir, label, "FOXN4.svg", sep = ""),
       plot = p, scale = 1, width = 15, height = 15)


muo_data <- get_tf_network(muo_data, tf='PTF1A')
p<- plot_tf_network(
    muo_data, 
    tf = 'PTF1A'
)
ggsave(filename = paste(output_dir, label, "PTF1A.svg", sep = ""),
       plot = p, scale = 1, width = 10, height = 10)
