...
for (out in output_dir) {
  for (i in 1:3) {
    ...
    coef <- coef[(coef$status == "OK") & (coef$term == "RegionPeripheral") &
                   (coef$q_value < 0.01) & (coef$normalized_effect <= -1) & (coef$num_cells_expressed >= 2000),
    ]
...
set1 <- unique(names(sort(table(gene_list$Name),decreasing = T)))
...
set2 <- (names(sort(table(gene_list$Name),decreasing = T)))

dput(intersect(set1,set2))
########################################################
########################################################
########################################################
########################################################
