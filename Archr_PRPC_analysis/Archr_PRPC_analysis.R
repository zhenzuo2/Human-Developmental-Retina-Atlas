...
UMAP$color <- as.numeric(PRPC$Days)
...
cA <- getCoAccessibility(ArchRProj = PRPC, corCutOff = 0.5, resolution = 1,
                         returnLoops = FALSE)
markerGenes = "PROX1"
p <- plotBrowserTrack(ArchRProj = PRPC, groupBy = "Days_", geneSymbol = markerGenes,
                      sizes = c(10, 1.5, 3, 4), upstream = 1e+05, downstream = 1e+05, loops = getCoAccessibility(PRPC),
                      , ylim = c(0, 1))
plotPDF(plotList = p, name = "Plot-Tracks-Marker-Genes-with-CoAccessibility.pdf",
        ArchRProj = PRPC, addDOC = FALSE, width = 5, height = 5)

# addPeak2GeneLinks
PRPC <- addPeak2GeneLinks(ArchRProj = PRPC, reducedDims = "IterativeLSI",
                          useMatrix = "GeneExpressionMatrix")
p2g <- getPeak2GeneLinks(ArchRProj = PRPC, corCutOff = 0.45, resolution = 1,
                         returnLoops = FALSE)
p <- plotBrowserTrack(ArchRProj = PRPC, groupBy = "Days_", geneSymbol = markerGenes,
                      upstream = 50000, downstream = 50000, loops = getPeak2GeneLinks(PRPC))
plotPDF(plotList = p, name = "Plot-Tracks-Marker-Genes-with-Peak2GeneLinks.pdf",
        ArchRProj = PRPC, addDOC = FALSE, width = 5, height = 5)
p <- plotPeak2GeneHeatmap(ArchRProj = PRPC, groupBy = "Days_")
svg("/storage/singlecell/zz4/fetal_bash/results/ArchR/PRPC/Plots/Plot-Tracks-Marker-Genes-with-Peak2GeneLinks.svg")
p
dev.off()

# markersPeaks
markersPeaks <- getMarkerFeatures(ArchRProj = PRPC, useMatrix = "PeakMatrix",
                                  groupBy = "Days_", bias = c("TSSEnrichment", "log10(nFrags)"), testMethod = "wilcoxon")
...
# addTrajectory
trajectory <- c("FW10", "FW13", "FW16", "FW19", "FW23")
PRPC <- addTrajectory(ArchRProj = PRPC, name = "MyeloidU", groupBy = "Days_",
...
markerMotifs
PRPC <- addGroupCoverages(ArchRProj = PRPC, groupBy = "Days_")
