...
for f in ${sample_id[@]}
do
    slurmtaco.sh -p short -m 5G -t 1 -- /storage/chen/home/zz4/anaconda3/envs/r/bin/Rscript scPred.R "${input_path}${f}.rds" "$output_results_path" "$f" "/storage/chen/data_share_folder/jinli/scpred/scPred_trainmodel_RNA_svmRadialWeights_scpred.rds" "/storage/singlecell/zz4/fetal_bash/data/Retina_fetal_sample_meta.csv";
done
